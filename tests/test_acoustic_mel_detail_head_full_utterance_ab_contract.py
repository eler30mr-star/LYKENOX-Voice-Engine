...
from lykenox_voice_engine.training.speech_acoustic_mel_detail_head_full_utterance_ab import (
    AUDIT_VERSION,
    OUTPUT_DIR_NAME,
    VALIDATION_INDICES,
    VARIANTS,
...
class AcousticMelDetailHeadFullUtteranceABContractTests(unittest.TestCase):
    def test_identity_and_variants_are_fixed(self) -> None:
        self.assertEqual(
            AUDIT_VERSION,
            "acoustic-frame-hidden-mel-detail-full-utterance-v4-2-ab-v1",
        )
    ...
    def test_gate_is_read_only_and_keeps_epoch_two_closed(self) -> None:
        source = inspect.getsource(run_frame_hidden_detail_full_utterance_v4_2_ab)
        for forbidden in (
            "optimizer.step",
            ".backward(",
    ...
    def test_gate_requires_exact_base_and_non_mel_identity(self) -> None:
        source = inspect.getsource(run_frame_hidden_detail_full_utterance_v4_2_ab)
        for key in (
            "duration_prediction",
            "regulated_durations",
        ...
        self.assertIn('detail_output["base_mel"]', source)
        self.assertIn("predicted_prosody_exact", source)
        self.assertIn("base_acoustic_identity_exact", source)
    ...
    def test_acceptance_is_perceptual_not_metric_only(self) -> None:
        ...
        self.assertIn("Ambiguous listening", source)
        self.assertIn(
