...
from lykenox_voice_engine.training.speech_vocoder_v7_train import ARTIFACT_DIR_NAME, TRAINER_CONTRACT_VERSION, V7_FIRST_EPOCH_GENERATOR_KWARGS, _run_config
...
class VocoderV7TrainingContractTests(unittest.TestCase):
    ...
    def test_run_config_hard_blocks_second_epoch_and_pins_smoked_model(self) -> None:
        ...
        self.assertEqual(config["generator_hyperparameters"], V7_FIRST_EPOCH_GENERATOR_KWARGS)
        self.assertEqual(V7_FIRST_EPOCH_GENERATOR_KWARGS["frame_channels"], 96)
        self.assertEqual(V7_FIRST_EPOCH_GENERATOR_KWARGS["upsample_channels"], (80, 56, 40))
        self.assertEqual(V7_FIRST_EPOCH_GENERATOR_KWARGS["residual_kernels"], (3, 7))
        self.assertEqual(V7_FIRST_EPOCH_GENERATOR_KWARGS["residual_dilations"], (1, 3))
    ...
    def test_checkpoint_roundtrip_preserves_source_free_contract_and_optimizer(self) -> None:
        torch.manual_seed(7)
        model = LykenoxVocoderGeneratorV7(**V7_FIRST_EPOCH_GENERATOR_KWARGS)
    ...
    def test_checkpoint_loader_rejects_false_source_free_claim(self) -> None:
        torch.manual_seed(11); model = LykenoxVocoderGeneratorV7(**V7_FIRST_EPOCH_GENERATOR_KWARGS)
        with tempfile.TemporaryDirectory() as temporary:
            ...
            payload = torch.load(path, map_location="cpu", weights_only=False); payload["sample_phase_conditioning"] = True; torch.save(payload, path)
            with self.assertRaisesRegex(RuntimeError, "sample_phase_conditioning"): load_v7_checkpoint(path)
