"""Exact-resume gate for the isolated acoustic mel-decoder refinement trainer."""
...
SMOKE_VERSION = "acoustic-mel-fidelity-exact-resume-smoke-v1"
...
def _protected_paths(root: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "persistent_isolated_last": training / "acoustic_mel_fidelity_v1" / "last.pt",
        "persistent_isolated_best": training / "acoustic_mel_fidelity_v1" / "best.pt",
    }
...
def _load_payload(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("resume smoke checkpoint payload is invalid")
    return payload


def run_isolated_mel_resume_smoke(root: Path) -> dict[str, object]:
    root = Path(root).resolve()
    protected = _protected_paths(root)
    before = {name: _sha256(path) for name, path in protected.items()}
    persistent_absent_before = (
        before["persistent_isolated_last"] is None
        and before["persistent_isolated_best"] is None
    )
    if not persistent_absent_before:
        raise RuntimeError(
            "resume smoke must run before persistent isolated mel training starts"
        )

    with tempfile.TemporaryDirectory(prefix="lykenox-mel-resume-") as temporary:
        ...
        direct_report = run_isolated_mel_fidelity_training(
            root,
            output_dir=direct_dir,
            **common,
        )
        split_first = run_isolated_mel_fidelity_training(
            root,
            output_dir=split_dir,
            max_updates_this_run=2,
            **common,
        )
        split_report = run_isolated_mel_fidelity_training(
            root,
            output_dir=split_dir,
            max_updates_this_run=2,
            **common,
        ...
        model_state_exact = _nested_equal(
            direct_payload["model_state"], split_payload["model_state"]
        )
        ...
        rerun_report = run_isolated_mel_fidelity_training(
            root,
            output_dir=split_dir,
            **common,
        )
    ...
    persistent_training_started = any(
        ...
        for name in ("persistent_isolated_last", "persistent_isolated_best")
    )
    ...
    return {
        ...
        **checks,
        ...
        "next_gate": (
            "authorize_isolated_mel_first_epoch_training"
            if status == "pass"
            else "fix_isolated_mel_exact_resume_before_training"
        ),
    }
...
def main() -> None:
    ...
    print(json.dumps(run_isolated_mel_resume_smoke(args.root), indent=2, ensure_ascii=False))
