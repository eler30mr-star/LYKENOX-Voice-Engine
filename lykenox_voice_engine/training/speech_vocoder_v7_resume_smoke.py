"""Exact-resume gate for V7 first-epoch training. Uses temporary artifact dirs only."""
...
SMOKE_VERSION = "vocoder-v7-first-epoch-exact-resume-smoke-v1"
...
def run_v7_resume_smoke(root: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="lykenox-v7-resume-") as tmp:
        tmp = Path(tmp); direct = tmp / "direct"; split = tmp / "split"
        ...
        a = _payload(direct / "last.pt"); b = _payload(split / "last.pt")
        meta_a, meta_b = a.get("training_metadata"), b.get("training_metadata")
