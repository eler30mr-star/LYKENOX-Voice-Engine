"""Persistent checkpoint contract for LYKENOX vocoder v5 training."""
...
V5_CHECKPOINT_VERSION = 1
V5_CHECKPOINT_KIND = "lykenox_v5_vocoder_training_checkpoint"
...
def build_v5_training_provenance(
    root: Path,
    *,
...
def _validate_provenance(provenance: dict[str, object]) -> None:
    ...
    for key, value in expected.items():
        if provenance.get(key) != value:
            raise RuntimeError(f"v5 checkpoint provenance mismatch: {key}")
...
def save_v5_checkpoint(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        "artifact_version": V5_CHECKPOINT_VERSION,
        "kind": V5_CHECKPOINT_KIND,
        "generator_architecture": VOCODER_GENERATOR_V5_ARCHITECTURE,
        ...
        "generator_hyperparameters": {
            ...
            "mel_conditioning_channels": generator.mel_conditioning_channels,
            "filter_bases": generator.filter_bases,
            "pulse_width_cycles": generator.pulse_width_cycles,
            "voiced_noise_floor": generator.voiced_noise_floor,
...
def load_v5_checkpoint(
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("Invalid LYKENOX v5 checkpoint payload")
    if payload.get("artifact_version") != V5_CHECKPOINT_VERSION:
        raise RuntimeError(f"Unsupported v5 checkpoint version: {payload.get('artifact_version')}")
    if payload.get("kind") != V5_CHECKPOINT_KIND:
        raise RuntimeError(f"Unexpected v5 checkpoint kind: {payload.get('kind')}")
    if payload.get("generator_architecture") != VOCODER_GENERATOR_V5_ARCHITECTURE:
        raise RuntimeError("v5 generator architecture mismatch")
    if payload.get("source_family") != LykenoxVocoderGeneratorV5.source_family:
        raise RuntimeError("v5 source family mismatch")
    if payload.get("explicit_sinusoidal_carrier") is not False:
        raise RuntimeError("v5 checkpoint unexpectedly declares sinusoidal carrier")
    if int(payload.get("deterministic_harmonics", -1)) != 0:
        raise RuntimeError("v5 checkpoint unexpectedly declares deterministic harmonics")
    if payload.get("discriminator_architecture") != DISCRIMINATOR_ARCHITECTURE:
        raise RuntimeError("v5 discriminator architecture mismatch")
    ...
    if not isinstance(config_payload, dict) or not isinstance(hyper, dict) or not isinstance(provenance, dict):
        raise RuntimeError("v5 checkpoint metadata is incomplete")
    ...
    if (config.mel_bins, config.sample_rate, config.hop_length) != (
        ...
        raise RuntimeError("v5 checkpoint is incompatible with active speech mel contract")

    generator = LykenoxVocoderGeneratorV5(config, **hyper)
    discriminator = LykenoxMultiScaleWaveformDiscriminator(
    ...
    if not isinstance(generator_state, dict) or not isinstance(discriminator_state, dict):
        raise RuntimeError("v5 checkpoint is missing model states")
