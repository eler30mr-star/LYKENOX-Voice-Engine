"""Bounded, exactly resumable persistent trainer for LYKENOX vocoder v4.4.

The stable checkpoint-selection objective is target-referenced:
reconstruction + envelope + spectral balance + local spectral contrast + F0-locked harmonic
exposure. Adversarial and feature-matching terms are training-only after warmup.

The default invocation budget stays below the local two-minute command ceiling and writes an
exact-resume checkpoint before returning incomplete.
"""
...
class EpochAccumulator:
    ...
    harmonic_exposure_sum: float = 0.0
    ...
    def from_payload(cls, payload: object, *, epoch: int) -> "EpochAccumulator":
        ...
        return cls(
            ...
            contrast_sum=float(payload.get("contrast_sum", 0.0)),
            harmonic_exposure_sum=float(payload.get("harmonic_exposure_sum", 0.0)),
    ...
    def to_payload(self, *, epoch: int) -> dict[str, object]:
        return {
            ...
            "contrast_sum": self.contrast_sum,
            "harmonic_exposure_sum": self.harmonic_exposure_sum,
...
def _generate(
    generator: LykenoxVocoderGeneratorV44,
...
def _base_losses(
    ...
    *,
    ...
    harmonic_exposure = target_relative_harmonic_exposure_loss(
        prediction,
        target,
        f0_hz,
        sample_rate=generator.config.sample_rate,
    ...
    total = (
        ...
        + harmonic_exposure_weight * harmonic_exposure
    )
    return total, {
        ...
        "balance": balance,
        "contrast": contrast,
        "harmonic_exposure": harmonic_exposure,
    }
    ...
    with torch.no_grad():
        for item in items:
            ...
            row = [
                ...
                float(losses["contrast"].detach()),
                float(losses["harmonic_exposure"].detach()),
            ]
            if not all(math.isfinite(value) for value in row):
                raise RuntimeError("Non-finite held-out v4.4 validation metric")
            for bucket, value in zip(values, row, strict=True):
                bucket.append(value)
    generator.train()
    means = [statistics.fmean(bucket) for bucket in values]
    score = (
        means[0]
        + envelope_weight * means[1]
        + balance_weight * means[2]
        + contrast_weight * means[3]
        + harmonic_exposure_weight * means[4]
    )
    return means[0], means[1], means[2], means[3], means[4], score
...
def _run_config(
    *,
    ...
    return {
        ...
        "generator_architecture": VOCODER_GENERATOR_V4_4_ARCHITECTURE,
        ...
        "harmonic_exposure_weight": float(harmonic_exposure_weight),
...
def _checkpoint_metadata(
    *,
    ...
    initial_metrics: tuple[float, float, float, float, float, float],
    best_metrics: tuple[float, float, float, float, float, float],
    best_epoch: int,
    ...
    metadata: dict[str, object] = {
        "purpose": "persistent_v4_4_dynamic_filter_hybrid_training",
        ...
        "checkpoint_semantics": (
            ...
            "next deterministic shuffled item; checkpoints occur only between complete updates"
        ),
    }
...
def _metrics_from_metadata(
    ...
    return (
        ...
        float(metadata[f"{prefix}_validation_local_spectral_contrast"]),
        float(metadata[f"{prefix}_validation_harmonic_exposure"]),
...
def _atomic_checkpoint(
    ...
    **kwargs: Any,
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_name(path.name + ".tmp")
    save_v4_4_checkpoint(temporary, generator, discriminator, **kwargs)
    os.replace(temporary, path)
...
def _save_state(
    ...
    *,
    ...
    metadata: dict[str, object],
    validation_metrics: tuple[float, float, float, float, float, float],
) -> None:
    reconstruction, envelope, balance, contrast, harmonic, score = validation_metrics
    _atomic_checkpoint(
        path,
        generator,
        discriminator,
        ...
        validation_harmonic_exposure_loss=harmonic,
...
def _latest_validation(
    ...
    return (
        ...
        float(row["validation_harmonic_exposure"]),
        float(row["validation_selection_score"]),
    )


def run_bounded_resumable_v4_4_training(
    root: Path,
    *,
    ...
    checkpoint_every_updates: int = 4,
    ...
    positive = (
        ...
        balance_weight,
        contrast_weight,
        harmonic_exposure_weight,
        gradient_clip_norm,
    )
    if min(positive) <= 0.0:
        raise ValueError("positive training hyperparameter required")
    ...
    if time_budget_seconds <= checkpoint_reserve_seconds + 8.0:
    ...
    provenance = build_v4_4_training_provenance(
        root,
        segment_mel_frames=segment_mel_frames,
        seed=seed,
    )
    ...
    if last_path.exists():
        generator, discriminator, resumed_payload = load_v4_4_checkpoint(last_path)
        if resumed_payload.get("training_provenance") != provenance:
            raise RuntimeError(
                "Existing v4.4 last.pt provenance differs from active dataset/features"
            )
        metadata = resumed_payload.get("training_metadata")
        if not isinstance(metadata, dict) or metadata.get("run_config") != run_config:
            raise RuntimeError(
                "Existing v4.4 last.pt training configuration differs from this command"
            )
        ...
        generator = LykenoxVocoderGeneratorV44().cpu().train()
    ...
    if resumed_payload is not None:
        ...
        if not isinstance(generator_state, dict) or not isinstance(
            ...
            raise RuntimeError("Cannot exactly resume v4.4 without both optimizer states")
    ...
    def save_interruption(reason: str) -> dict[str, object]:
        ...
        progress = {
            ...
            "best_epoch": best_epoch,
            "best_validation_selection_score": round(best_metrics[5], 6),
            "best_validation_harmonic_exposure": round(best_metrics[4], 6),
    ...
    while epoch <= max_epochs:
        ...
        if next_item_offset > len(train_conditioned):
            raise RuntimeError("v4.4 resume offset exceeds deterministic epoch item count")

        while next_item_offset < len(train_conditioned):
            elapsed = time.perf_counter() - started
            estimated_update = max(update_times[-4:] or [4.0])
            is_last_item = next_item_offset == len(train_conditioned) - 1
            required = (
                checkpoint_reserve_seconds
                + estimated_update
                + (validation_reserve_seconds if is_last_item else 0.0)
            )
            ...
            if not bool(torch.isfinite(generator_total)):
                raise RuntimeError("Non-finite v4.4 generator loss")
            generator_total.backward()
            generator_norm = torch.nn.utils.clip_grad_norm_(
                generator.parameters(),
                max_norm=gradient_clip_norm,
            )
            if not bool(torch.isfinite(generator_norm)):
                raise RuntimeError("Non-finite v4.4 generator gradient norm")
            ...
            if adversarial_active:
                ...
                if not bool(torch.isfinite(discriminator_loss)):
                    raise RuntimeError("Non-finite v4.4 discriminator loss")
                ...
                if not bool(torch.isfinite(discriminator_norm)):
                    raise RuntimeError("Non-finite v4.4 discriminator gradient norm")
            ...
            metric_values = {
                ...
                "harmonic_exposure": float(base["harmonic_exposure"].detach()),
            ...
            if not all(
                ...
                raise RuntimeError("Non-finite v4.4 training metric")

            accumulator.reconstruction_sum += metric_values["reconstruction"]
            accumulator.envelope_sum += metric_values["envelope"]
            accumulator.balance_sum += metric_values["balance"]
            accumulator.contrast_sum += metric_values["contrast"]
            accumulator.harmonic_exposure_sum += metric_values["harmonic_exposure"]
            accumulator.generator_total_sum += metric_values["total"]
            ...
            if global_step % checkpoint_every_updates == 0:
                _save_state(
                    ...
                    validation_metrics=_latest_validation(
                        history,
                        initial_metrics,
                    ),
                )
        ...
        row = {
            ...
            "train_harmonic_exposure": accumulator.harmonic_exposure_sum / count,
            ...
            "validation_harmonic_exposure": validation[4],
            "validation_selection_score": validation[5],
        }
        history.append(row)

        if validation[5] < best_metrics[5] - min_delta:
    ...
    training_improved = best_metrics[5] < initial_metrics[5]
    envelope_improved = best_metrics[1] < initial_metrics[1]
    harmonic_exposure_improved = best_metrics[4] < initial_metrics[4]
    completed_pass = (
        training_improved
        and envelope_improved
        and harmonic_exposure_improved
    ...
    report = {
        ...
        "initial_validation": {
            ...
            "harmonic_exposure": round(initial_metrics[4], 6),
            "selection_score": round(initial_metrics[5], 6),
        },
        "best_validation": {
            ...
            "harmonic_exposure": round(best_metrics[4], 6),
            "selection_score": round(best_metrics[5], 6),
        },
        "training_improved": training_improved,
        "envelope_improved": envelope_improved,
        "harmonic_exposure_improved": harmonic_exposure_improved,
        "train_items": train_items,
        ...
        "next_gate": (
            "run_v4_4_full_utterance_oracle_acceptance"
            if completed_pass
            else "inspect_v4_4_training_failure_before_any_more_training"
        ),
    }
