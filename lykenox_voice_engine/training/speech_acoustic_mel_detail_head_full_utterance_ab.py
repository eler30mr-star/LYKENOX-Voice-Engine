"""Read-only full-utterance A/B gate for the one-epoch frame-hidden mel detail head.

The accepted acoustic-v2 base and vocoder-v4.2 remain immutable. Three fixed held-out
utterances are rendered on teacher durations. Target-prosody pairs isolate the detail
head's mel contribution, while predicted-prosody pairs verify the same change in the real
acoustic-to-vocoder route. No training, duration modification, gain, EQ or denoising is
permitted here; full-utterance listening is the acceptance gate.
"""
...
from lykenox_voice_engine.training.speech_acoustic_mel_detail_head_artifact import (
    ...
    detail_head_output_dir,
    file_sha256,
    load_mel_detail_head_checkpoint,
)
...
AUDIT_VERSION = "acoustic-frame-hidden-mel-detail-full-utterance-v4-2-ab-v1"
VALIDATION_INDICES = (0, 1, 2)
OUTPUT_DIR_NAME = "acoustic_mel_detail_head_v1_full_utterance_v4_2_ab"
VARIANTS = (
    "v4_2_oracle",
    "base_mel_target_prosody",
    "detail_mel_target_prosody",
    "base_mel_predicted_prosody",
    "detail_mel_predicted_prosody",
)
...
def _protected_paths(root: Path) -> dict[str, Path]:
    ...
    detail_dir = detail_head_output_dir(root)
    return {
        "acoustic_v2_best": training / "acoustic_frame_context_v2" / "best.pt",
        "detail_best": detail_dir / "best.pt",
        "detail_last": detail_dir / "last.pt",
...
def _require_detail_epoch1_identity(payload: dict[str, object], *, base_sha256: str) -> None:
    run_config = payload.get("run_config")
    metadata = payload.get("training_metadata")
    if not isinstance(run_config, dict) or not isinstance(metadata, dict):
        raise RuntimeError("detail-head checkpoint is missing run/training metadata")
    ...
    if payload.get("base_checkpoint_sha256") != base_sha256:
        raise RuntimeError("detail-head checkpoint base SHA mismatch")
    if run_config.get("trainer_contract_version") != TRAINER_CONTRACT_VERSION:
        raise RuntimeError("detail-head checkpoint trainer identity mismatch")
    if run_config.get("trainable_surface") != "detail_head_only":
        raise RuntimeError("detail-head checkpoint trainable surface mismatch")
    if int(run_config.get("hard_epoch_limit", -1)) != HARD_EPOCH_LIMIT:
        raise RuntimeError("detail-head checkpoint hard epoch gate mismatch")
    if int(payload.get("epoch", -1)) != 2 or int(payload.get("next_item_offset", -1)) != 0:
        raise RuntimeError("detail-head checkpoint is not closed exactly after epoch 1")
    history = metadata.get("history")
    if not isinstance(history, list) or len(history) != 1:
        raise RuntimeError("detail-head checkpoint must contain exactly one completed epoch")
    if int(metadata.get("best_epoch", -1)) != 1:
        raise RuntimeError("detail-head best checkpoint was not selected from epoch 1")


def run_frame_hidden_detail_full_utterance_v4_2_ab(root: Path) -> dict[str, object]:
    root = Path(root).resolve()
    protected = _protected_paths(root)
    required = ("acoustic_v2_best", "detail_best", "detail_last", "v4_2_best")
    missing = [name for name in required if not protected[name].exists()]
    if missing:
        raise FileNotFoundError(f"Missing required detail-head A/B checkpoints: {missing}")
    ...
    if base_path != protected["acoustic_v2_best"]:
        raise RuntimeError("detail-head A/B base checkpoint path contract changed")
    base_sha = file_sha256(base_path)
    candidate, candidate_payload = load_mel_detail_head_checkpoint(
        protected["detail_best"],
        base_checkpoint=base_path,
    )
    _require_detail_epoch1_identity(candidate_payload, base_sha256=base_sha)

    base = candidate.base_model
    base.cpu().eval()
    candidate.cpu().eval()
    if any(parameter.requires_grad for parameter in base.parameters()):
        raise RuntimeError("accepted acoustic base became trainable during detail-head A/B")
    base_identity_model, base_payload = load_acoustic_prosody_checkpoint(base_path)
    base_run = base_payload.get("run_config")
    base_acoustic_identity_exact = (
        isinstance(base_run, dict)
        ...
        and base_identity_model.config.frame_context_version == FRAME_CONTEXT_VERSION
    )
    if not base_acoustic_identity_exact:
        raise RuntimeError("detail-head A/B requires accepted acoustic frame-context-v2 base")
    ...
    if not v4_2_identity_exact:
        raise RuntimeError("detail-head A/B requires accepted vocoder v4.2")
    if (
        ...
        raise RuntimeError("detail-head/v4.2 feature contract mismatch")
    ...
    if len(dataset) <= max(VALIDATION_INDICES):
        raise RuntimeError("not enough held-out validation items for detail-head A/B")
    ...
    with torch.inference_mode():
        for audit_index, dataset_index in enumerate(VALIDATION_INDICES, start=1):
            ...
            if batch.f0_hz is None or batch.voiced is None:
                raise RuntimeError("detail-head A/B requires cached target F0/voicing")
            ...
            detail_output = candidate(batch.token_ids, batch.token_mask, batch.durations)
            exact_non_mel_outputs = all(
                torch.equal(base_output[key], detail_output[key])
                for key in (
                    "duration_prediction",
                    "regulated_durations",
                    "f0_prediction_hz",
            ...
            base_mel_identity = torch.equal(base_output["mel"], detail_output["base_mel"])
            structural_checks.extend((exact_non_mel_outputs, base_mel_identity))
            if not exact_non_mel_outputs or not base_mel_identity:
                raise RuntimeError("detail head changed non-mel outputs or lost base mel identity")

            base_conditioning = prepare_speech_vocoder_conditioning(base_output)
            detail_conditioning = prepare_speech_vocoder_conditioning(detail_output)
            predicted_prosody_exact = (
                torch.equal(
                    base_conditioning.f0_hz[:, :frames],
                    detail_conditioning.f0_hz[:, :frames],
                )
                and torch.equal(
                    base_conditioning.voiced[:, :frames],
                    detail_conditioning.voiced[:, :frames],
                )
            )
            structural_checks.append(predicted_prosody_exact)
            if not predicted_prosody_exact:
                raise RuntimeError("prepared predicted prosody changed through detail head")
            ...
            detail_mel = detail_conditioning.mel[:, :frames]
            ...
            active_mask = _reference_active_frame_mask(reference, candidate.config.hop_length)
            ...
            variant_inputs = {
                ...
                "detail_mel_target_prosody": (detail_mel, target_f0, target_voiced),
                "base_mel_predicted_prosody": (base_mel, pred_f0, pred_voiced),
                "detail_mel_predicted_prosody": (detail_mel, pred_f0, pred_voiced),
            }

            prefix = f"{audit_index:02d}"
            reference_path = output_dir / f"{prefix}_reference.wav"
            sf.write(str(reference_path), reference.numpy(), candidate.config.sample_rate, subtype="PCM_16")
            ...
            for variant_name in VARIANTS:
                ...
                sf.write(str(wav_path), wave.numpy(), candidate.config.sample_rate, subtype="PCM_16")
            ...
            items.append(
                {
                    ...
                    "base_mel_metrics": {
                        ...
                        for key, value in _mel_metrics(base_mel[0], target_mel[0], centers_hz=centers).items()
                    },
                    "detail_mel_metrics": {
                        key: round(float(value), 6)
                        for key, value in _mel_metrics(detail_mel[0], target_mel[0], centers_hz=centers).items()
                    },
                    "variants": variants,
                }
            )
    ...
    report: dict[str, object] = {
        ...
        "detail_head_architecture": MEL_DETAIL_HEAD_ARCHITECTURE_V1,
        "detail_checkpoint_identity_exact": True,
        "base_acoustic_identity_exact": base_acoustic_identity_exact,
        ...
        "listening_order": (
            ...
            "detail_mel_target_prosody -> base_mel_predicted_prosody -> "
            "detail_mel_predicted_prosody"
        ),
        "acceptance_rule": (
            "Detail head is acceptable only if full held-out speech is audibly clearer or more "
            "intelligible than base without new metallic, nasal, noisy, harsh, grid-like, or "
            "consonant-loss artifacts. Metrics may reject but cannot accept. Ambiguous listening "
            "does not authorize epoch 2."
        ),
        "next_gate": "listen_frame_hidden_detail_vs_base_before_any_epoch2",
    }
...
def main() -> None:
    ...
    print(json.dumps(run_frame_hidden_detail_full_utterance_v4_2_ab(args.root), indent=2, ensure_ascii=False))
