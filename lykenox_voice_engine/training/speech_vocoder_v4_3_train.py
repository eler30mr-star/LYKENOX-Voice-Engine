"""Bounded, exactly resumable persistent trainer for LYKENOX vocoder v4.3.

This trainer is deliberately separate from v4.1/v4.2. Persistent execution is not
authorized until the exact-resume smoke passes. The stable target-referenced selection
objective is reconstruction + log-mel envelope + broad spectral balance + local spectral
contrast. Adversarial/feature matching is training-only and begins after warmup.
"""
...
from lykenox_voice_engine.training.speech_vocoder_v4_3_artifact import (
    build_v4_3_training_provenance,
    load_v4_3_checkpoint,
    save_v4_3_checkpoint,
)
...
DEFAULT_VALIDATION_RESERVE_SECONDS = 18.0
...
def _generate(generator: LykenoxVocoderGeneratorV43, item: ConditionedSegment) -> torch.Tensor:
    return generator(
    ...
    *,
    ...
    with torch.no_grad():
        for item in items:
            ...
            values = [float(losses[key].detach()) for key in ("reconstruction", "envelope", "balance", "contrast")]
            if not all(math.isfinite(value) for value in values):
                raise RuntimeError("Non-finite held-out v4.3 validation metric")
            reconstruction_values.append(values[0])
            envelope_values.append(values[1])
            balance_values.append(values[2])
            contrast_values.append(values[3])
    generator.train()
    ...
    contrast_mean = statistics.fmean(contrast_values)
    score = (
        reconstruction_mean
        + envelope_weight * envelope_mean
        + balance_weight * balance_mean
        + contrast_weight * contrast_mean
    )
    return reconstruction_mean, envelope_mean, balance_mean, contrast_mean, score
...
def _checkpoint_metadata(
    *,
    ...
    metadata: dict[str, object] = {
        "purpose": "persistent_v4_3_mel_filtered_carrier_training",
...
def _atomic_checkpoint(
    ...
    **kwargs: Any,
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_name(path.name + ".tmp")
    save_v4_3_checkpoint(temporary, generator, discriminator, **kwargs)
    os.replace(temporary, path)
...
def run_bounded_resumable_v4_3_training(
    root: Path,
    *,
    ...
    provenance = build_v4_3_training_provenance(root, segment_mel_frames=segment_mel_frames, seed=seed)
    ...
    artifact_dir = (
        ...
        else root / "models" / "lykenox_identity" / "training" / "vocoder_mel_filtered_carrier_v4_3"
    )
    ...
    if last_path.exists():
        generator, discriminator, resumed_payload = load_v4_3_checkpoint(last_path)
        if resumed_payload.get("training_provenance") != provenance:
            raise RuntimeError("Existing v4.3 last.pt provenance differs from active dataset/features")
        metadata = resumed_payload.get("training_metadata")
        if not isinstance(metadata, dict) or metadata.get("run_config") != run_config:
            raise RuntimeError("Existing v4.3 last.pt training configuration differs from this command")
        ...
        generator = LykenoxVocoderGeneratorV43().cpu().train()
    ...
    if resumed_payload is not None:
        ...
        if not isinstance(generator_state, dict) or not isinstance(discriminator_state, dict):
            raise RuntimeError("Cannot exactly resume v4.3 without both optimizer states")
    ...
    def save_interruption(reason: str) -> dict[str, object]:
        ...
        progress = {
            ...
            "best_validation_local_spectral_contrast": round(best_metrics[3], 6),
    ...
    while epoch <= max_epochs:
        ...
        if next_item_offset > len(train_conditioned):
            raise RuntimeError("v4.3 resume offset exceeds deterministic epoch item count")
        ...
        while next_item_offset < len(train_conditioned):
            ...
            if not bool(torch.isfinite(generator_total)):
                raise RuntimeError("Non-finite v4.3 generator loss")
            generator_total.backward()
            generator_norm = torch.nn.utils.clip_grad_norm_(generator.parameters(), max_norm=gradient_clip_norm)
            if not bool(torch.isfinite(generator_norm)):
                raise RuntimeError("Non-finite v4.3 generator gradient norm")
            ...
            if adversarial_active:
                ...
                if not bool(torch.isfinite(discriminator_loss)):
                    raise RuntimeError("Non-finite v4.3 discriminator loss")
                discriminator_loss.backward()
                discriminator_norm = torch.nn.utils.clip_grad_norm_(discriminator.parameters(), max_norm=gradient_clip_norm)
                if not bool(torch.isfinite(discriminator_norm)):
                    raise RuntimeError("Non-finite v4.3 discriminator gradient norm")
            ...
            if not all(math.isfinite(value) for value in (*values.values(), adversarial_value, feature_matching_value, discriminator_value)):
                raise RuntimeError("Non-finite v4.3 training metric")
        ...
        row = {
            ...
            "train_adversarial": accumulator.adversarial_sum / adversarial_count if accumulator.adversarial_count else 0.0,
            "train_feature_matching": accumulator.feature_matching_sum / adversarial_count if accumulator.adversarial_count else 0.0,
            "train_discriminator": accumulator.discriminator_sum / adversarial_count if accumulator.adversarial_count else 0.0,
    ...
    contrast_improved = best_metrics[3] < initial_metrics[3]
    completed_pass = training_improved and envelope_improved and contrast_improved and best_path.exists()
    report = {
        ...
        "local_spectral_contrast_improved": contrast_improved,
        "train_items": train_items,
        ...
        "next_gate": (
            "run_v4_3_full_utterance_oracle_acceptance"
            if completed_pass
            else "inspect_v4_3_training_failure_before_any_more_training"
        ),
    }
...
def main() -> None:
    ...
    print(json.dumps(run_bounded_resumable_v4_3_training(
        args.root,
