"""Persistent checkpoint contract for LYKENOX vocoder v4.4 training.

V4.4 artifacts are separate from v4.1-v4.3. The checkpoint captures generator,
discriminator, optimizer and RNG state so bounded CPU runs resume exactly without silently
changing dataset, feature, loss or architecture identity.
"""
...
V4_4_CHECKPOINT_VERSION = 1
V4_4_CHECKPOINT_KIND = "lykenox_v4_4_vocoder_training_checkpoint"
...
def build_v4_4_training_provenance(
    root: Path,
    *,
...
def _validate_provenance(provenance: dict[str, object]) -> None:
    ...
    for key, value in expected.items():
        if provenance.get(key) != value:
            raise RuntimeError(f"v4.4 checkpoint provenance mismatch: {key}")


def save_v4_4_checkpoint(
    path: Path,
    ...
    *,
    ...
    validation_harmonic_exposure_loss: float | None,
    ...
    payload: dict[str, Any] = {
        "artifact_version": V4_4_CHECKPOINT_VERSION,
        "kind": V4_4_CHECKPOINT_KIND,
        "generator_architecture": VOCODER_GENERATOR_V4_4_ARCHITECTURE,
        ...
        "validation_harmonic_exposure_loss": validation_harmonic_exposure_loss,
...
def load_v4_4_checkpoint(
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("Invalid LYKENOX v4.4 checkpoint payload")
    if payload.get("artifact_version") != V4_4_CHECKPOINT_VERSION:
        raise RuntimeError(
            f"Unsupported v4.4 checkpoint version: {payload.get('artifact_version')}"
        )
    if payload.get("kind") != V4_4_CHECKPOINT_KIND:
        raise RuntimeError(f"Unexpected v4.4 checkpoint kind: {payload.get('kind')}")
    if payload.get("generator_architecture") != VOCODER_GENERATOR_V4_4_ARCHITECTURE:
        raise RuntimeError("v4.4 generator architecture mismatch")
    if payload.get("discriminator_architecture") != DISCRIMINATOR_ARCHITECTURE:
        raise RuntimeError("v4.4 discriminator architecture mismatch")
    ...
    if (
        ...
        raise RuntimeError("v4.4 checkpoint metadata is incomplete")
    ...
    if (config.mel_bins, config.sample_rate, config.hop_length) != (
        ...
        raise RuntimeError("v4.4 checkpoint is incompatible with active speech mel contract")

    generator = LykenoxVocoderGeneratorV44(config, **hyper)
    discriminator = LykenoxMultiScaleWaveformDiscriminator(
    ...
    if not isinstance(generator_state, dict) or not isinstance(discriminator_state, dict):
        raise RuntimeError("v4.4 checkpoint is missing model states")
