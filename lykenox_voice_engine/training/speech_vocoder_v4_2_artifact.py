"""Persistent checkpoint contract for LYKENOX vocoder v4.2 training.

V4.2 artifacts are deliberately separate from v4.1.  The checkpoint captures the exact
model/discriminator/optimizer/RNG state required for bounded CPU training to resume without
silently changing dataset, feature, loss, or architecture identity.
"""
...
V4_2_CHECKPOINT_VERSION = 1
V4_2_CHECKPOINT_KIND = "lykenox_v4_2_vocoder_training_checkpoint"
...
def build_v4_2_training_provenance(
    root: Path,
    *,
...
def _validate_provenance(provenance: dict[str, object]) -> None:
    ...
    for key, value in expected.items():
        if provenance.get(key) != value:
            raise RuntimeError(f"v4.2 checkpoint provenance mismatch: {key}")


def save_v4_2_checkpoint(
    path: Path,
    ...
    *,
    ...
    payload: dict[str, Any] = {
        "artifact_version": V4_2_CHECKPOINT_VERSION,
        "kind": V4_2_CHECKPOINT_KIND,
        "generator_architecture": VOCODER_GENERATOR_V4_2_ARCHITECTURE,
        ...
        "validation_envelope_loss": None if validation_envelope_loss is None else float(validation_envelope_loss),
...
def load_v4_2_checkpoint(
    path: Path,
) -> tuple[LykenoxVocoderGeneratorV42, LykenoxMultiScaleWaveformDiscriminator, dict[str, object]]:
    payload = torch.load(Path(path), map_location="cpu", weights_only=False)
    if not isinstance(payload, dict):
        raise RuntimeError("Invalid LYKENOX v4.2 checkpoint payload")
    if payload.get("artifact_version") != V4_2_CHECKPOINT_VERSION:
        raise RuntimeError(
            f"Unsupported v4.2 checkpoint version: {payload.get('artifact_version')}"
        )
    if payload.get("kind") != V4_2_CHECKPOINT_KIND:
        raise RuntimeError(f"Unexpected v4.2 checkpoint kind: {payload.get('kind')}")
    if payload.get("generator_architecture") != VOCODER_GENERATOR_V4_2_ARCHITECTURE:
        raise RuntimeError("v4.2 generator architecture mismatch")
    if payload.get("discriminator_architecture") != DISCRIMINATOR_ARCHITECTURE:
        raise RuntimeError("v4.2 discriminator architecture mismatch")
    ...
    if not isinstance(config_payload, dict) or not isinstance(hyper, dict) or not isinstance(provenance, dict):
        raise RuntimeError("v4.2 checkpoint metadata is incomplete")
    ...
    if (config.mel_bins, config.sample_rate, config.hop_length) != (
        ...
        raise RuntimeError("v4.2 checkpoint is incompatible with active speech mel contract")

    generator = LykenoxVocoderGeneratorV42(config, **hyper)
    discriminator = LykenoxMultiScaleWaveformDiscriminator(
    ...
    if not isinstance(generator_state, dict) or not isinstance(discriminator_state, dict):
        raise RuntimeError("v4.2 checkpoint is missing model states")
