"""Exact-resume gate for the frame-hidden mel detail-head trainer."""
...
SMOKE_VERSION = "acoustic-frame-hidden-mel-detail-exact-resume-smoke-v1"
...
def _protected_paths(root: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "rejected_postnet_last": training / "acoustic_mel_postnet_v1" / "last.pt",
        ...
        "persistent_detail_last": training / "acoustic_mel_detail_head_v1" / "last.pt",
        "persistent_detail_best": training / "acoustic_mel_detail_head_v1" / "best.pt",
    }
...
def _load_payload(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise RuntimeError("detail-head resume checkpoint payload is invalid")
    return payload


def run_mel_detail_head_resume_smoke(root: Path) -> dict[str, object]:
    root = Path(root).resolve()
    protected = _protected_paths(root)
    before = {name: _sha256(path) for name, path in protected.items()}
    persistent_absent_before = (
        before["persistent_detail_last"] is None
        and before["persistent_detail_best"] is None
    )
    if not persistent_absent_before:
        raise RuntimeError("detail-head resume smoke must run before persistent training starts")

    base_path = base_checkpoint_path(root)
    if not base_path.exists():
        raise FileNotFoundError(f"accepted acoustic-v2 checkpoint not found: {base_path}")
    base_sha_before = file_sha256(base_path)

    with tempfile.TemporaryDirectory(prefix="lykenox-detail-head-resume-") as temporary:
        ...
        direct_report = run_mel_detail_head_training(root, output_dir=direct_dir, **common)
        split_first = run_mel_detail_head_training(
            root,
            output_dir=split_dir,
            max_updates_this_run=2,
            **common,
        )
        split_report = run_mel_detail_head_training(
            root,
            output_dir=split_dir,
            max_updates_this_run=2,
            **common,
        ...
        detail_head_state_exact = _nested_equal(
            direct_payload["detail_head_state"], split_payload["detail_head_state"]
        )
        ...
        rerun_report = run_mel_detail_head_training(root, output_dir=split_dir, **common)
        after_rerun_sha = _sha256(split_last)
        gate_checkpoint_unchanged_on_rerun = before_rerun_sha == after_rerun_sha
        epoch1_gate_reached = (
            direct_report.get("status") == "gate_reached"
            and split_report.get("status") == "gate_reached"
            and rerun_report.get("status") == "gate_reached"
        )
    ...
    base_sha_after = file_sha256(base_path)
    ...
    persistent_training_started = any(
        ...
        for name in ("persistent_detail_last", "persistent_detail_best")
    )
    ...
    checks = {
        ...
        "detail_head_state_exact": detail_head_state_exact,
    ...
    return {
        ...
        **checks,
        ...
        "next_gate": (
            "authorize_frame_hidden_detail_first_epoch_training"
            if status == "pass"
            else "fix_frame_hidden_detail_exact_resume_before_training"
        ),
    }
...
def main() -> None:
    ...
    print(json.dumps(run_mel_detail_head_resume_smoke(args.root), indent=2, ensure_ascii=False))
