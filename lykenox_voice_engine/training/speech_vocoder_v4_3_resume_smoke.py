"""Exact-resume smoke for the bounded LYKENOX v4.3 persistent trainer.

Runs the same four deterministic CPU updates two ways in temporary artifact directories:
1) one invocation with four updates; 2) two invocations with two updates each. The gate
passes only when generator, discriminator, both optimizers, RNG, epoch/offset and global
step are bit-exact. Production v4.3 and historical v4.2 artifacts are not used as output.
"""
...
from lykenox_voice_engine.training.speech_vocoder_v4_3_train import (
    TRAINER_CONTRACT_VERSION,
    run_bounded_resumable_v4_3_training,
)


SMOKE_VERSION = "vocoder-v4-3-exact-resume-smoke-v1"
...
def run_v4_3_resume_smoke(root: Path) -> dict[str, object]:
    ...
    v4_2_before = _sha256_if_exists(v4_2_best)
    ...
    with tempfile.TemporaryDirectory(prefix="lykenox_v43_resume_") as temporary:
        temp = Path(temporary)
        direct_dir = temp / "direct"
        split_dir = temp / "split"

        direct = run_bounded_resumable_v4_3_training(
            root,
            artifact_dir_override=direct_dir,
            max_updates_this_run=4,
            **common,
        )
        split_first = run_bounded_resumable_v4_3_training(
            root,
            artifact_dir_override=split_dir,
            max_updates_this_run=2,
            **common,
        )
        split_second = run_bounded_resumable_v4_3_training(
            root,
            artifact_dir_override=split_dir,
            max_updates_this_run=2,
            **common,
    ...
    v4_2_after = _sha256_if_exists(v4_2_best)
    v4_2_unchanged = v4_2_before == v4_2_after
    all_exact = all(bool(value) for value in checks.values()) and v4_2_unchanged
    return {
        ...
        **checks,
        "v4_2_checkpoint_present": v4_2_before is not None,
        "v4_2_checkpoint_unchanged": v4_2_unchanged,
        "temporary_artifacts_removed": True,
        "persistent_v4_3_training_started": False,
        "next_gate": (
            "start_bounded_resumable_v4_3_persistent_training"
            if all_exact
            else "fix_v4_3_resume_contract_before_persistent_training"
        ),
    }
...
def main() -> None:
    ...
    print(json.dumps(run_v4_3_resume_smoke(args.root), indent=2, ensure_ascii=False))
