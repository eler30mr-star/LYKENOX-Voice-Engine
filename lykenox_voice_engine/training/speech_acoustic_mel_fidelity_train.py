"""Bounded exactly-resumable first-epoch trainer for isolated acoustic mel refinement.

This experiment starts from accepted ``acoustic_frame_context_v2/best.pt`` and changes only
``mel_decoder.*`` parameters.  Encoder, duration, frame-context and F0/voicing parameters
must remain bit-exact with the base checkpoint.  Training is hard-blocked after one epoch;
full-utterance v4.2 A/B is required before any further refinement can be considered.
"""
...
from lykenox_voice_engine.training.speech_acoustic_mel_fidelity_artifact import (
    ...
    isolated_output_dir,
    load_isolated_mel_checkpoint,
    require_frozen_state_exact,
    save_isolated_mel_checkpoint,
)
...
TRAINER_CONTRACT_VERSION = "acoustic-mel-fidelity-first-epoch-resumable-v1"
...
def _run_config(
    *,
    ...
    return {
        ...
        "artifact_version": ISOLATED_MEL_ARTIFACT_VERSION,
        ...
        "trainable_parameter_prefix": TRAINABLE_PREFIX,
...
def _validation(model, dataset: LykenoxAlignedSpeechDataset, *, batch_size: int) -> dict[str, float]:
    sums = {
        "total": 0.0,
        "mel_l1": 0.0,
        "centered_shape": 0.0,
    ...
    with torch.no_grad():
        for start in range(0, len(dataset), batch_size):
            ...
            if not torch.equal(output["regulated_durations"], batch.durations):
                raise RuntimeError("isolated mel validation changed teacher durations")
            ...
            if not all(math.isfinite(value) for value in current.values()):
                raise RuntimeError("non-finite isolated mel validation metric")
            for name, value in current.items():
                sums[name] += value
            count += 1
    if count < 1:
        raise RuntimeError("isolated mel validation dataset is empty")
...
def _accumulate(accumulator: dict[str, object], snapshot: dict[str, float]) -> None:
    ...
    if not isinstance(sums, dict):
        raise RuntimeError("isolated mel accumulator is invalid")
...
def _accumulator_means(accumulator: dict[str, object]) -> dict[str, float]:
    ...
    if count < 1 or not isinstance(sums, dict):
        raise RuntimeError("cannot summarize empty isolated mel epoch")
    return {name: float(value) / count for name, value in sums.items()}


def run_isolated_mel_fidelity_training(
    root: Path,
    *,
    ...
    if batch_size < 1 or learning_rate <= 0.0 or grad_clip <= 0.0:
        raise ValueError("invalid isolated mel optimizer configuration")
    ...
    if not base_path.exists():
        raise FileNotFoundError(f"accepted acoustic v2 base checkpoint not found: {base_path}")
    base_sha = file_sha256(base_path)
    base_model, base_payload = load_acoustic_prosody_checkpoint(base_path)
    base_run = base_payload.get("run_config")
    if not isinstance(base_run, dict) or base_run.get("trainer_contract_version") != BASE_TRAINER_CONTRACT_VERSION:
        raise RuntimeError("isolated mel refinement requires accepted acoustic v2 trainer identity")
    if base_model.config.frame_context_version != FRAME_CONTEXT_VERSION:
        raise RuntimeError("isolated mel refinement requires accepted frame-context architecture")
    ...
    if base_payload.get("training_provenance") != current_provenance:
    ...
    output_dir = Path(output_dir).resolve() if output_dir is not None else isolated_output_dir(root)
    ...
    if last_path.exists():
        model, payload = load_isolated_mel_checkpoint(
            last_path,
        ...
        if not isinstance(metadata, dict):
            raise RuntimeError("isolated mel checkpoint is missing training_metadata")
    ...
    def metadata_payload() -> dict[str, object]:
        return {
            ...
            "only_mel_decoder_trainable": True,
    ...
    if epoch > HARD_EPOCH_LIMIT or history:
        ...
        return {
            ...
            "next_gate": (
                "run_isolated_mel_full_utterance_v4_2_ab"
                if best_epoch == 1
                else "reject_isolated_mel_candidate_without_perceptual_run"
            ),
        }
    ...
    if next_item_offset < 0 or next_item_offset > len(order):
        raise RuntimeError("isolated mel next_item_offset is invalid")
    ...
    while next_item_offset < len(order):
        ...
        output = model(batch.token_ids, batch.token_mask, batch.durations)
        if not torch.equal(output["regulated_durations"], batch.durations):
            raise RuntimeError("isolated mel trainer changed teacher durations")
        result = acoustic_mel_fidelity_loss(
            output["mel"],
            batch.mel,
            batch.mel_mask,
        ...
        if not torch.isfinite(result.total):
            raise RuntimeError(f"non-finite isolated mel loss at global step {global_step}")
        result.total.backward()
        grad_norm = torch.nn.utils.clip_grad_norm_(model.mel_decoder.parameters(), grad_clip)
        if not math.isfinite(float(grad_norm)):
            raise RuntimeError(f"non-finite isolated mel gradient at global step {global_step}")
        optimizer.step()
        require_frozen_state_exact(model, base_model)
    ...
    validation = _validation(model, val_dataset, batch_size=batch_size)
...
def main() -> None:
    ...
    print(json.dumps(run_isolated_mel_fidelity_training(args.root), indent=2, ensure_ascii=False))
