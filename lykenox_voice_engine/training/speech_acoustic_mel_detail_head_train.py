"""Bounded exactly-resumable first-epoch trainer for the frame-hidden mel detail head.

The accepted acoustic-v2 base stays immutable. Only ``detail_head.*`` parameters are
optimized. Training is hard-blocked after one epoch; full-utterance v4.2 A/B remains the
only acceptance path after the numerical gate.
"""
...
TRAINER_CONTRACT_VERSION = "acoustic-frame-hidden-mel-detail-first-epoch-resumable-v1"
...
def _run_config(
    *,
    ...
    return {
        ...
        "trainable_surface": "detail_head_only",
...
def _loss(candidate, batch):
    ...
    if not torch.equal(output["regulated_durations"], batch.durations):
        raise RuntimeError("detail-head trainer changed teacher durations")
    ...
    with torch.no_grad():
        for start in range(0, len(dataset), batch_size):
            ...
            if not all(math.isfinite(value) for value in current.values()):
                raise RuntimeError("non-finite detail-head validation metric")
            for name, value in current.items():
                sums[name] += value
            count += 1
    if count < 1:
        raise RuntimeError("detail-head validation dataset is empty")
...
def _accumulate(accumulator: dict[str, object], snapshot: dict[str, float]) -> None:
    ...
    if not isinstance(sums, dict):
        raise RuntimeError("detail-head accumulator is invalid")
...
def _accumulator_means(accumulator: dict[str, object]) -> dict[str, float]:
    ...
    if count < 1 or not isinstance(sums, dict):
        raise RuntimeError("cannot summarize empty detail-head epoch")
    return {name: float(value) / count for name, value in sums.items()}


def run_mel_detail_head_training(
    root: Path,
    *,
    ...
    if batch_size < 1 or learning_rate <= 0.0 or grad_clip <= 0.0:
        raise ValueError("invalid detail-head optimizer configuration")
    ...
    if base_run.get("trainer_contract_version") != BASE_TRAINER_CONTRACT_VERSION:
        raise RuntimeError("detail-head trainer requires accepted acoustic-v2 trainer identity")
    if base.config.frame_context_version != FRAME_CONTEXT_VERSION:
        raise RuntimeError("detail-head trainer requires accepted frame-context architecture")

    duration_root = find_clean_duration_root(root)
    provenance = build_acoustic_prosody_provenance(root, duration_root=duration_root, config=base.config)
    ...
    train_dataset = LykenoxAlignedSpeechDataset(
        root, "train", base.config, duration_root=duration_root, include_pitch_targets=False
    )
    val_dataset = LykenoxAlignedSpeechDataset(
        root, "val", base.config, duration_root=duration_root, include_pitch_targets=False
    )
    ...
    output_dir = Path(output_dir).resolve() if output_dir is not None else detail_head_output_dir(root)
    ...
    if last_path.exists():
        candidate, payload = load_mel_detail_head_checkpoint(
            last_path,
        ...
        optimizer = torch.optim.AdamW(
            candidate.detail_head.parameters(), lr=learning_rate, weight_decay=0.0
        )
        ...
        if not isinstance(metadata, dict):
            raise RuntimeError("detail-head checkpoint is missing training metadata")
    ...
    if not names or any(not name.startswith("detail_head.") for name in names):
        raise RuntimeError("detail-head trainer may optimize only detail_head parameters")
    ...
    def metadata_payload() -> dict[str, object]:
        return {
            ...
            "only_detail_head_trainable": True,
    ...
    if epoch > HARD_EPOCH_LIMIT or history:
        return {
            ...
            "next_gate": (
                "run_frame_hidden_detail_full_utterance_v4_2_ab"
                if best_epoch == 1
                else "reject_frame_hidden_detail_without_perceptual_run"
            ),
        }
    ...
    if next_item_offset < 0 or next_item_offset > len(order):
        raise RuntimeError("detail-head next_item_offset is invalid")
    ...
    while next_item_offset < len(order):
        ...
        if not torch.isfinite(result.total):
            raise RuntimeError(f"non-finite detail-head loss at global step {global_step}")
        result.total.backward()
        grad_norm = torch.nn.utils.clip_grad_norm_(candidate.detail_head.parameters(), grad_clip)
        if not math.isfinite(float(grad_norm)):
            raise RuntimeError(f"non-finite detail-head gradient at global step {global_step}")
        optimizer.step()
...
def main() -> None:
    ...
    print(json.dumps(run_mel_detail_head_training(args.root), indent=2, ensure_ascii=False))
