"""Exact-resume smoke for the bounded LYKENOX v4.2 persistent trainer.
...
2. two invocations with two updates each, resuming from ``last.pt``.

The gate passes only when model states, discriminator states, both optimizer states, torch
RNG state, epoch/offset and global step are bit-exact.  Production v4.2 and historical v4.1
artifact directories are not used by this smoke.
"""
...
from lykenox_voice_engine.training.speech_vocoder_v4_2_train import (
    TRAINER_CONTRACT_VERSION,
    run_bounded_resumable_v4_2_training,
)


SMOKE_VERSION = "vocoder-v4-2-exact-resume-smoke-v1"
...
def run_v4_2_resume_smoke(root: Path) -> dict[str, object]:
    ...
    v4_1_before = _sha256_if_exists(v4_1_best)
    ...
    with tempfile.TemporaryDirectory(prefix="lykenox_v42_resume_") as temporary:
        temp = Path(temporary)
        direct_dir = temp / "direct"
        split_dir = temp / "split"

        direct = run_bounded_resumable_v4_2_training(
            root,
            artifact_dir_override=direct_dir,
            max_updates_this_run=4,
            **common,
        )
        split_first = run_bounded_resumable_v4_2_training(
            root,
            artifact_dir_override=split_dir,
            max_updates_this_run=2,
            **common,
        )
        split_second = run_bounded_resumable_v4_2_training(
            root,
            artifact_dir_override=split_dir,
            max_updates_this_run=2,
            **common,
    ...
    v4_1_after = _sha256_if_exists(v4_1_best)
    v4_1_unchanged = v4_1_before == v4_1_after
    all_exact = all(bool(value) for value in checks.values()) and v4_1_unchanged
    ...
    return {
        ...
        **checks,
        "v4_1_checkpoint_present": v4_1_before is not None,
        "v4_1_checkpoint_unchanged": v4_1_unchanged,
        "temporary_artifacts_removed": True,
        "persistent_v4_2_training_started": False,
        "next_gate": (
            "start_bounded_resumable_v4_2_persistent_training"
            if all_exact
            else "fix_v4_2_resume_contract_before_persistent_training"
        ),
    }
...
def main() -> None:
    ...
    print(json.dumps(run_v4_2_resume_smoke(args.root), indent=2, ensure_ascii=False))
