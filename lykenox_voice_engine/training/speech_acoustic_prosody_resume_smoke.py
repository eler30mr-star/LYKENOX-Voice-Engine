"""Exact split/resume equivalence gate for persistent LYKENOX acoustic prosody training."""
...
SMOKE_VERSION = "acoustic-prosody-exact-resume-smoke-v1"
...
def run_resume_smoke(root: Path) -> dict[str, object]:
    ...
    smoke_root = root / "models" / "lykenox_identity" / "training" / "acoustic_prosody_resume_smoke"
    ...
    reference_result = run_acoustic_prosody_training(
        root,
        output_dir=reference_dir,
        max_updates_this_run=10,
        **common,
    )
    split_first = run_acoustic_prosody_training(
        root,
        output_dir=split_dir,
        max_updates_this_run=3,
        **common,
    )
    split_second = run_acoustic_prosody_training(
        root,
        output_dir=split_dir,
    ...
    optimizer_exact = _nested_equal(
        reference_payload["optimizer_state"],
        resumed_payload["optimizer_state"],
    )
    rng_exact = _nested_equal(reference_payload["torch_rng_state"], resumed_payload["torch_rng_state"])
    position_exact = (
        int(reference_payload["epoch"]) == int(resumed_payload["epoch"])
        and int(reference_payload["next_item_offset"]) == int(resumed_payload["next_item_offset"])
        and int(reference_payload["global_step"]) == int(resumed_payload["global_step"])
    )
    metadata_exact = _nested_equal(
        reference_payload["training_metadata"],
        resumed_payload["training_metadata"],
    )
    run_config_exact = reference_payload["run_config"] == resumed_payload["run_config"]
    provenance_exact = reference_payload["training_provenance"] == resumed_payload["training_provenance"]
    ...
    output_exact = all(torch.equal(reference_output[key], resumed_output[key]) for key in output_keys)
    max_output_delta = max(
        float((reference_output[key] - resumed_output[key]).abs().max())
        for key in ("mel", "f0_prediction_hz", "f0_log_prediction", "voicing_logits", "duration_prediction")
    )
    ...
    report = {
        ...
        **checks,
        ...
        "next_gate": (
            ...
            if status == "pass"
            else "fix_acoustic_prosody_resume_contract"
        ),
    }
