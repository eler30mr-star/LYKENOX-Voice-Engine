"""Pure DSP oracle for the owned minimum-phase renderer.

This diagnostic isolates renderer/excitation quality from every learned model. It loads three
owned validation utterances, derives a reference log-magnitude directly from each waveform,
converts that magnitude to the renderer's one-sided real cepstrum, and renders with the owned
full-utterance F0/voicing/periodicity conditioning already attached to the utterance.

No model is instantiated. No optimizer, backward pass, parameter update, checkpoint read/write,
training, gain normalization, EQ, denoise, enhancement, or duration modification is allowed.
The output is raw FLOAT WAV for direct A/B listening under LYX-POL-001.
"""
...
ORACLE_VERSION = "owned-minimum-phase-renderer-oracle-v1"
...
def _reference_log_magnitude(waveform: torch.Tensor, *, frame_count: int) -> tuple[torch.Tensor, int]:
    """Return reference log-magnitude as [frame_count, 513] on renderer geometry."""
    ...
    if int(waveform.numel()) != expected_samples:
        raise ValueError(
            "oracle waveform length must equal frame_count * renderer hop length"
        )
    ...
    # _centered_stft_magnitude: [batch, bins, analysis_frames].
...
def run_minimum_phase_oracle(
    ...
    *,
    ...
    if split == "train":
        raise ValueError("renderer oracle must use a held-out split, not train")
    ...
    with torch.no_grad():
        for utterance in utterances:
            ...
            if (
                ...
                raise RuntimeError("held-out oracle pitch conditioning length mismatch")
            ...
            if int(prediction.numel()) != expected_samples:
                raise RuntimeError("oracle renderer violated exact output-length contract")
            if prediction.shape != reference.shape:
                raise RuntimeError("oracle prediction/reference shape mismatch")

            stem = _safe_name(utterance.utterance_id)
            prediction_path = output_dir / f"{stem}__oracle_prediction.wav"
            ...
            items.append(
                {
                    ...
                    "noise_seed": NOISE_SEED,
                    "oracle_prediction": str(prediction_path),
                    "reference": str(reference_path),
                }
            )

    report: dict[str, object] = {
        "status": "ready_for_oracle_listening",
        ...
        "cepstral_order": CEPSTRAL_ORDER,
        "noise_seed": NOISE_SEED,
        "reference_spectral_source": "owned_reference_waveform_centered_stft_log_magnitude",
        "pitch_source": "owned_full_utterance_pitch_cache_v1_unmodified",
        "model_used": False,
        ...
        "items": items,
        "next_action": "listen_to_raw_oracle_prediction_vs_reference_pairs_before_training",
    }
