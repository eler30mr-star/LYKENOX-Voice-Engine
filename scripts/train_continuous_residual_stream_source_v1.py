"""One-command train-and-render entrypoint for the non-overlapping residual stream source V1."""
...
from lykenox_voice_engine.training.speech_vocoder_continuous_stream_source_train_v1 import (
    ...
    DEFAULT_TRAIN_ITEMS,
    DEFAULT_VAL_ITEMS,
    train_continuous_stream_source_v1,
)
from scripts.render_continuous_residual_stream_source_v1 import (
    render_heldout_continuous_stream_source_v1,
)
...
def main() -> None:
    ...
    training = train_continuous_stream_source_v1(
        args.root,
    ...
    listening = render_heldout_continuous_stream_source_v1(
        args.root,
        heldout_items=args.val_items,
        checkpoint=Path(str(training["best_checkpoint"])),
    )
    print(json.dumps({
        "status": "continuous_residual_stream_source_train_and_listen_complete",
        "root_correction": "remove_512_256_duplicate_sample_authority_and_hop_grid_overlap_inconsistency",
        "training": training,
        "listening": listening,
