...
def main() -> None:
    ...
    report = prepare_dataset(args.train_csv, args.val_csv, args.output_dir)
    print(json.dumps(report, indent=2, ensure_ascii=False))


def prepare_dataset(train_csv: Path, val_csv: Path, output_dir: Path) -> dict[str, object]:
    """Copy WAV files and write Coqui metadata CSVs."""

    wav_dir = output_dir / "wavs"
    wav_dir.mkdir(parents=True, exist_ok=True)
    train_rows = _copy_rows(train_csv, wav_dir)
    val_rows = _copy_rows(val_csv, wav_dir)
...
def _copy_rows(csv_path: Path, wav_dir: Path) -> list[dict[str, str]]:
    rows = []
